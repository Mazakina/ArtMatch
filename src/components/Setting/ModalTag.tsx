...
export default function ModalTag({
  ...
  function handleOnSkillChange(e) {
    ...
    if (e.target.checked === false) {
      const newArray = habilidades.filter((h) => {
          ...
          >
            {skillList.map((skill) => {
              return (
                <Checkbox
                  key={skill.id}
                  isChecked={habilidades.includes(skill.id)}
                  value={skill.id}
                  onChange={(e) => handleOnSkillChange(e)}
                  _checked={{ color: "#FFEB80", border: "1px solid #FFEB80" }}
            ...
            <Button
              border={"1px solid #FFEB80"}
              _hover={{ color: "#000", background: "#FFEB80" }}
              color="#FFEB80"
              bg="none"
              w="100%"
              p=".5rem"
