...
const SkillsTagSection = React.memo<SkillTagSectionProps>(
  function SkillsTagSection({ habilidades = [], skillList = [] }) {
    const displayArray = skillList.filter((skill) => {
    ...
    return (
      ...
      >
        {displayArray.map((tag) => {
          return <SkillTag name={tag.name} key={tag.id} />
        })}
      </Flex>
    )
  },
