...
export default function SocialOptions<SocialOptionsProps>({icon, text, value,placeholder, onChange,defaultValue = ''}){
  ...
  return(
    <Flex width="100%" align="center" justify='center'  mt='25px'>
      ...
      <Input  type={text.toLowerCase()=='telefone'? 'number': 'text'} onChange={(event)=>{getValue(event)}} placeholder={placeholder} fontSize="14px" h='28px' w='50%' ml='35px' borderRadius='2px' bg='#0B0B0B' border='1px solid #BEBEBE' value={value} defaultValue={defaultValue} />
    </Flex>
  )
}