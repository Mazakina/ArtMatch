  ...
  return(
    ...
    {searchItems[0]?
      ...
      searchItems.map((item:ItemProps) =>{
        return(
          <ListItem key={item.id||item.user} as={Link} href={`${process.env.NEXT_PUBLIC_BASE_URL}/posts/${item.id||item.user}`} cursor='pointer ' mb='.3rem'  borderBottom='1px solid #c5c5c5'pb={'.3rem'} display={'flex'} alignItems={'center'} fontSize='1rem'>
