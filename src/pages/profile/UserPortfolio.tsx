...
export default function UserPortfolio({ name, avatar, albums, posts = [] }) {
  ...
  const filterFunction = (post) => {
    const isMidiaOk = post.midia === midiaFilter || midiaFilter === ''
    const isAlbumOk = post.albumName === albumFilter || albumFilter === ''
    if (isMidiaOk && isAlbumOk) {
      return true
    }
    return false
  }
  const newPostsArray = posts.filter((post) => filterFunction(post))

  return (
    <Box w="100%" id="portfolio">
      <Flex>
        <Text mt="25px" ml="6vw" h="29px">
          Filtre por:
        </Text>
        <Select
          h="29px"
          ...
          cursor="pointer"
          onChange={(event) => {
            setMidiaFilter(event.target.value)
          }}
          value={midiaFilter}
        >
        ...
        >
          {albums
            ? albums.map((album) => {
                return (
                  <option key={album.albumRef} value={album.AlbumName}>
                    {album.albumName}
                  </option>
                )
              })
