...
export default function Posts({ postData, slug }: PostsProps) {
    ...
    if (user) {
      setLiked(
        currentPost.likes.filter(
          (like) => like.toString() == user.ref.toString()
        )
      );
    }
  ...
  const handleLikeButton = (e) => {
    ...
    if (!liked[0]) {
      ...
      setCurrentPost({
        ...currentPost,
        likes: [...currentPost.likes, user.ref],
      });
    }
    if (liked[0]) {
      likeFunctions.dislike(user, slug, postData.user.name);
      const filteredLikes = currentPost.likes.filter(
        (f) => f.toString() != user.ref.toString()
      );
      ...
      >
        ...
        >
          <ChakraLink
            ...
            alignItems="center"
          >
            <Flex mr="1rem" cursor={"pointer"} alignItems="center">
              <Avatar
                mr="12px"
                width="44px"
                height="44px"
              ...
              <Box>
                <Text fontSize="20px" color="#fff">
                  {" "}
                  {capitalizeFirstLetter(currentPost.user.name)}{" "}
                </Text>
              </Box>
              <Button
                onClick={(e) => {
                  handleFavoriteUserButton(e);
                }}
                fontSize="12px"
                ml="auto"
                mt="auto"
            ...
            <Flex mt="16px" alignItems="center" justifyContent="space-between">
              <Flex>
                <Icon
                  ...
                  color={liked[0] ? "#f8473b" : "white"}
                  aria-active={liked[0] ? "true" : "false"}
                  aria-label="curtir Post"
                  fontSize="25px"
                  cursor={"pointer"}
                  as={liked[0] ? AiFillHeart : AiOutlineHeart}
                />
                <Text ml=".5rem" color="white">
                  {currentPost.likes ? currentPost.likes.length : ""}
                </Text>
              </Flex>
              <Icon
                onClick={(e) => {
                  handleFavoritePostButton(e);
                }}
                fontSize="25px"
                cursor={"pointer"}
                aria-active={favoritedPost ? "true" : "false"}
                aria-label="adicionar/remover post aos favoritos"
                as={favoritedPost ? BsBookmarkHeartFill : BsBookmarkPlus}
              />
            </Flex>
          </Flex>
          ...
          <Flex flexDir="column">
            <Text mb="1rem" fontSize="20px">
              Mais de {capitalizeFirstLetter(currentPost?.user.name)}{" "}
            </Text>
            <Grid
              bg="#1a1a1a"
              padding=".5rem"
                ...
                return (
                  ...
                  >
                    <Tooltip label={post.title} aria-label="A tooltip">
                      ...
                      >
                        <Link
                          aria-label={`visitar postagem ${post.title}`}
...
export const favPostFunctions = {
  delete: (session, slug) => {
    if (session) {
      Api.delete("/_lib/imgur/favoritePost", {
        data: {
          id: slug,
          ...session,
        },
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/_lib/imgur/imgurGet`,
    {
      method: "GET",
      headers: {
  ...
  postData.otherPosts = postData.otherPosts.filter((post) => {
