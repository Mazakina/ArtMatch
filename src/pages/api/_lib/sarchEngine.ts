  ...
  try {
    ...
    if (searchType === 'byUser') {
      const filteredData = userArray.filter((item) => {
        return (
          levenshtein.get(item.user.toLowerCase(), searchTerm.toLowerCase()) <=
          4
        )
      })
      filteredData.sort((a, b) => {
        const aScore = Math.min(levenshtein.get(a.user, searchTerm))
        const bScore = Math.min(levenshtein.get(b.user, searchTerm))
    ...
    if (searchType === 'byPosts') {
      ...
      newDataArray.map((postsByUser) =>
        postsByUser.map((post) => filteredData.push(post)),
      )
