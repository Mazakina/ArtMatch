...
export default async function favoriteUser(
  ...
  async function getUser() {
    const user: userProps = await fauna.query(
      q.Get(q.Match(q.Index('user_by_email'), req.body.data.email)),
    )
    return user
  }
  async function getPostOwner() {
    const postOwner: userProps = await fauna.query(
      q.Get(q.Match(q.Index('user_by_usuario'), req.body.postOwnerName)),
    )
    return postOwner
  }
    ...
    try {
      await fauna
        .query(
          q.If(
            q.Not(
              q.Exists(q.Match(q.Index('favorite_users_by_user_id'), user.ref)),
            ),
            q.Create(q.Collection('favorite_users'), {
              data: {
                userId: user.ref,
                ...
                data: {
                  favoritedUsers: q.Append(
                    ...
                    q.Filter(
                      ...
                      q.Lambda('i', q.Not(q.Equals(q.Var('i'), postOwner.ref))),
                    ),
                  ),
                },
              },
    ...
    await fauna
      .query(
        q.Update(
          ...
          {
            data: {
              favoritedUsers: q.Filter(
                q.Select(
                  ['data', 'favoritedUsers'],
                  q.Get(
                    q.Match(q.Index('favorite_users_by_user_id'), user.ref),
                  ),
                ),
                q.Lambda('i', q.Not(q.Equals(q.Var('i'), postOwner.ref))),
              ),
      ...
      .catch(() => res.status(404).end('collection not found'))
    return
  }
