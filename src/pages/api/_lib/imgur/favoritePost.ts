...
export default async function favoritePost(
  ...
  if (req.method === 'POST') {
    ...
    try {
      const favoritePosts = await fauna
        .query(q.Get(q.Match(q.Index('favorite_posts_by_user_id'), user.ref)))
    ...
    try {
      await fauna
        .query(
          q.If(
            q.Not(
              q.Exists(q.Match(q.Index('favorite_posts_by_user_id'), user.ref)),
            ),
            q.Create(q.Collection('favorite_posts'), {
              data: {
                userId: user.ref,
                ...
                data: {
                  favoritedPosts: q.Append(
                    ...
                    q.Filter(
                      ...
                      q.Lambda('i', q.Not(q.Equals(q.Var('i'), req.body.id))),
                    ),
                  ),
                },
              },
      ...
      await fauna
        .query(
          q.Update(
            ...
            {
              data: {
                favoritedPosts: q.Filter(
                  q.Select(
                    ['data', 'favoritedPosts'],
                    q.Get(
                      q.Match(q.Index('favorite_posts_by_user_id'), user.ref),
                    ),
                  ),
                  q.Lambda('i', q.Not(q.Equals(q.Var('i'), req.body.id))),
                ),
